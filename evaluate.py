...
def main(
    ...
    def evaluate(
        ...
        with torch.no_grad():
            if args.adapter == 'Prefix':
                # Prefix 专用：手动注入 past_key_values
                batch_size = input_ids.shape[0]
                past_key_values = model.get_prompt(batch_size)
                past_key_values = tuple(
                    tuple(pkv.half() for pkv in pkv_tuple) if isinstance(pkv_tuple, tuple) else pkv_tuple.half()
    ...
    def evaluate(
        ...
        with torch.no_grad():
            if args.adapter == 'Prefix':
                # Prefix/Prompt/P-Tuning：都用 model.generate + use_cache=False
    ...
    for idx, data in enumerate(dataset):
        ...
        flag = False
        '''
        if args.dataset.lower() in ['aqua', 'arc-c', 'arc-e', 'obqa']:
            ...
            if abs(label - predict) <= miss:
                ...
                flag = True
        '''
        # 处理 answer1/answer2/answer3/answer4 格式的答案
        predict = extract_answer_from_output(outputs)
        if predict.lower() == label.lower():
            correct += 1
            flag = True
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['ARC-e','ARC-c','OBQA'],
                        required=True)
...
def extract_answer_number(args, sentence: str) -> float:
    ...
    if dataset in ["ARC-e","ARC-c","OBQA"]:
...
def extract_answer_from_output(sentence: str) -> str:
    """提取答案"""
    sentence = sentence.lower().strip()
    # 匹配 answer1, answer2, answer3, answer4
    match = re.search(r'answer\d+', sentence)
    if match:
        return match.group()
    # 如果没有找到 answer\d+，尝试匹配 A/B/C/D 并转换
    match = re.findall(r'\b[A-D]\b', sentence.upper())
    if match:
        return f"answer{ord(match[-1]) - ord('A') + 1}"
    return ""
